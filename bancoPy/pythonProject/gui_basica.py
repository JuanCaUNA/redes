...
class SinpeGUIBasica:
    ...
    def create_widgets(self):
        ...
        # Título
        title_label = ttk.Label(main_frame, text="🏦 SINPE Banking System", 
                               font=('Arial', 16, 'bold'))
        title_label.grid(row=0, column=0, columnspan=3, pady=(0, 20))
        
        # Estado del servidor
        self.server_status_label = ttk.Label(main_frame, text="🔄 Verificando servidor...", 
                                           foreground="orange")
    ...
    def check_server_status(self):
        ...
        def check():
            try:
                response = requests.get(f"{self.api_base}/users", verify=False, timeout=3)
                if response.status_code == 200:
                    self.server_status_label.config(text="✅ Servidor conectado", foreground="green")
                    self.log_message("✅ Conexión con servidor establecida")
                else:
                    self.server_status_label.config(text="⚠️ Servidor con problemas", foreground="orange")
                    self.log_message("⚠️ Servidor responde pero hay problemas")
            except Exception as e:
                self.server_status_label.config(text="❌ Servidor desconectado", foreground="red")
    ...
    def api_request(self, method, endpoint, data=None):
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                self.log_message(f"❌ {method} {endpoint} - Error {response.status_code}")
                return None
...
¡Comienza seleccionando una opción del menú!
        ...
        if data and data.get('success'):
            ...
            if transactions:
                for i, trans in enumerate(transactions, 1):
                    ...
                    content += f"   📤 Desde: Cuenta {trans.get('from_account_id', 'N/A')}\n"
                    content += f"   📥 Hacia: Cuenta {trans.get('to_account_id', 'N/A')}\n"
    ...
    def create_user_dialog(self):
        ...
        # Campos
        ttk.Label(frame, text="👤 Crear Nuevo Usuario", font=('Arial', 12, 'bold')).grid(row=0, column=0, columnspan=2, pady=(0, 20))
        ...
        ttk.Button(button_frame, text="✅ Crear", command=create_user).grid(row=0, column=0, padx=5)
    ...
    def create_account_dialog(self):
        ...
        # Campos
        ttk.Label(frame, text="🏦 Crear Nueva Cuenta", font=('Arial', 12, 'bold')).grid(row=0, column=0, columnspan=2, pady=(0, 20))
        
        ttk.Label(frame, text="ID de Usuario:").grid(row=1, column=0, sticky=tk.W, pady=5)
        ...
        ttk.Label(frame, text="Saldo Inicial:").grid(row=3, column=0, sticky=tk.W, pady=5)
        ...
        def create_account():
            try:
                ...
                messagebox.showwarning("Datos Inválidos", "Verifique que los valores sean correctos")
        
        # Botones
        button_frame = ttk.Frame(frame)
        button_frame.grid(row=4, column=0, columnspan=2, pady=20)
        
        ttk.Button(button_frame, text="✅ Crear", command=create_account).grid(row=0, column=0, padx=5)
        ttk.Button(button_frame, text="❌ Cancelar", command=dialog.destroy).grid(row=0, column=1, padx=5)
    ...
    def transfer_dialog(self):
        ...
        # Campos
        ttk.Label(frame, text="💸 Realizar Transferencia", font=('Arial', 12, 'bold')).grid(row=0, column=0, columnspan=2, pady=(0, 20))
        
        ttk.Label(frame, text="Cuenta Origen (ID):").grid(row=1, column=0, sticky=tk.W, pady=5)
        from_entry = ttk.Entry(frame, width=30)
        from_entry.grid(row=1, column=1, pady=5)
        
        ttk.Label(frame, text="Cuenta Destino (ID):").grid(row=2, column=0, sticky=tk.W, pady=5)
        ...
        ttk.Button(button_frame, text="💸 Transferir", command=make_transfer).grid(row=0, column=0, padx=5)
        ttk.Button(button_frame, text="❌ Cancelar", command=dialog.destroy).grid(row=0, column=1, padx=5)
