...
class BankingLoggerService:
    ...
    def log_transaction(self, transaction_data: Dict[str, Any], event_type: str = "transaction"):
        ...
        log_entry = {
            ...
            "description": transaction_data.get("description", "")[:100]  # Truncate long descriptions
        }
    ...
    def log_security_event(self, event_type: str, details: Dict[str, Any], severity: str = "WARNING"):
        ...
        if severity.upper() == "CRITICAL":
            self.security_logger.critical(f"Security Event: {event_type}", extra={"json_data": log_entry})
        elif severity.upper() == "ERROR":
            self.security_logger.error(f"Security Event: {event_type}", extra={"json_data": log_entry})
        else:
            self.security_logger.warning(f"Security Event: {event_type}", extra={"json_data": log_entry})
    ...
    def log_fraud_detection(self, transaction_id: str, risk_score: int, 
                           alerts: list, action_taken: str):
    ...
    def log_inter_bank_communication(self, bank_code: str, operation: str, 
        ...
        if success:
            self.app_logger.info(f"Inter-bank communication successful: {operation} with bank {bank_code}")
        else:
            self.error_logger.error(f"Inter-bank communication failed: {operation} with bank {bank_code}", 
                                  extra={"json_data": log_entry})

    def log_balance_update(self, account_number: str, previous_balance: float, 
                          new_balance: float, transaction_id: str, update_type: str):
    ...
    def log_error(self, error_type: str, error_message: str, 
        ...
        self.error_logger.error(f"{error_type}: {error_message}", extra={"json_data": log_entry})
    ...
    def log_system_startup(self, version: str, config: Dict):
        ...
        log_entry = {
            ...
            "version": version,
            "config": {
                "database_url": config.get("database_url", "").replace(
                    config.get("database_url", "").split("://")[1].split("@")[0], "***"
                ) if "@" in config.get("database_url", "") else config.get("database_url", ""),
