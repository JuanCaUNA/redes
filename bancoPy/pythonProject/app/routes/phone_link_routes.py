...
def get_phone_links():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "data": [link.to_dict() for link in phone_links]}
        )
...
def create_phone_link():
    ...
    try:
        ...
        if existing_phone_link:
            return (
                jsonify({"error": "Phone number already linked to another account"}),
                400,
            )

        # Check if account already has a phone link
        existing_account_link = PhoneLink.query.filter_by(
        ...
        phone_link = PhoneLink(
            account_number=data["account_number"], phone=data["phone"]
        )

        db.session.add(phone_link)
        db.session.commit()

        return jsonify({"success": True, "data": phone_link.to_dict()}), 201
...
def get_phone_link(link_id):
    ...
    try:
        ...
        return jsonify({"success": True, "data": phone_link.to_dict()})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def update_phone_link(link_id):
    ...
    try:
        ...
        if "account_number" in data:
            ...
            existing_link = PhoneLink.query.filter_by(
                account_number=data["account_number"]
            ).first()
            if existing_link and existing_link.id != link_id:
                return jsonify({"error": "Account already has a phone link"}), 400
...
def delete_phone_link(link_id):
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Phone link deleted successfully"})
